...
const { scheduledItems } = await useTermine()

// Rail nur auf Übersichts-Pages zeigen, NICHT auf Detail-Pages
// (`/friendlies/[id]`, `/challenges/[id]`, `/spieler/[id]`, etc.). Auf
// Detail-Pages ist der User focused auf ein konkretes Item — andere
// Termine danebenzulegen ist eher Ablenkung als Kontext, und die Page
// gewinnt die volle Main-Breite zurück.
const usesRail = computed(() => Object.keys(route.params).length === 0)

const railNextMatch = computed(() => scheduledItems.value[0] ?? null)
const railUpcomingRest = computed(() => scheduledItems.value.slice(1))
const termineEmpty = computed(() => scheduledItems.value.length === 0)
...
<template>
  <div
    ...
    :class="loggedIn
      ? usesRail
        ? 'lg:grid lg:grid-cols-[256px_minmax(0,1fr)] xl:grid-cols-[256px_minmax(0,1fr)_320px]'
  ...
  >
    ...
    >
      ...
      <!-- Termine-Sections (Als-nächstes-Card + Geplant-Liste) sind als
           eigene Komponente extrahiert, weil sie auch in der Home-Page
           inline für Mobile gebraucht werden (< xl, wo der Rail nicht
           sichtbar ist). -->
      <TermineSections :next-match="railNextMatch" :upcoming-rest="railUpcomingRest" />
    </aside>
